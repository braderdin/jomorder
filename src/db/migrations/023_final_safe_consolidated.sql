-- Start: Phase 56 - Final Safe Consolidated DDL (master idempotent)
-- Fasal 13 (DDL Sync) + Fasal 7 S1 (RLS) + Fasal 11 (IPv4 pooler ?pgbouncer=false).
-- Gabungkan DDL selamat dari 001 + 004 + 005 + 017 + 021 + rls_policies TANPA:
--   * INSERT ke schema_migrations (jadual tidak wujud -> ERROR 42P01).
--   * RLS WITH CHECK rujuk OLD (ERROR 42P01).
--   * CHECK constraint jenis_diskaun 'PERCENT'/'AMOUNT' (drift vs 'PERATUS'/'TANAH').
--   * auth.uid()::text::bigint (merchant_telegram_id adalah TEXT -> RLS block).
...
-- ============================================================
-- 4. RLS owner-update guard (NO OLD/NEW conflict, FIXED TEXT cast)
-- ============================================================
ALTER TABLE senarai_kedai ENABLE ROW LEVEL SECURITY;
...
-- Start: Phase 56 - RLS authenticated merchant isolation (TEXT cast fix)
DROP POLICY IF EXISTS "merchant_manage_own_shop" ON senarai_kedai;
CREATE POLICY "merchant_manage_own_shop"
  ON senarai_kedai FOR ALL TO authenticated
...
CREATE POLICY "merchant_own_orders"
  ON rekod_pesanan FOR ALL TO authenticated
  USING (kedai_id IN (SELECT id FROM senarai_kedai WHERE merchant_telegram_id = auth.uid()::text))
  WITH CHECK (kedai_id IN (SELECT id FROM senarai_kedai WHERE merchant_telegram_id = auth.uid()::text));
-- End: Phase 56 - RLS authenticated merchant isolation
...
-- ============================================================
-- 8. Coupon sweep indexes (serap 017, TANPA schema_migrations)
-- ============================================================
CREATE INDEX IF NOT EXISTS idx_kempen_diskaun_sweep ON kempen_diskaun (status_aktif, tamat_pada);
CREATE INDEX IF NOT EXISTS idx_kempen_diskaun_kod_kedai ON kempen_diskaun (kedai_id, kod_kupon);

-- ============================================================
-- 9. Seed kawasan operasi
-- ============================================================
...
-- End: Phase 56 - Final Safe Consolidated DDL