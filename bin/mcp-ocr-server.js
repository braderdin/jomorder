...
rl.on("line", (line) => {
  ...
  if (msg.method === "initialize") {
    ...
    send({ jsonrpc: "2.0", id: msg.id, result: { tools: [{ name: "ocr_image", description: "Extract text from an image file path using Tesseract OCR", inputSchema: { type: "object", properties: { image_path: { type: "string" }, lang: { type: "string" }, psm: { type: "number" } }, required: ["image_path"] } }] } });
    ...
    if (name === "ocr_image") {
      ...
      const psm = (typeof args.psm === "number") ? args.psm : 6;
      Tesseract.recognize(args.image_path, { lang, oem: 1, psm })
